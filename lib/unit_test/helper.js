 ...
 */
function test(sdkSrc, platforms, callback) {
	let sdkPath,
		tasks = [], // WILL REMOVE ON COMPLETION
		results = {};

	tasks.push(function(next) {
		// install new SDK and delete old test app in parallel
		async.parallel([ // FIND WORKAROUND FOR PARALLEL ASYNC IN PROMISES. PROMISE.ALL?
			function(cb) {
				if(sdkSrc) {
					console.log('Installing SDK');
					AppcUtil.installSDK(sdkSrc, cb);
				} else {
					cb();
				}
			},
	...
	new PromiseSequence() // JUST A PLACEHOLDER FOR THE PROMISE START RIGHT NOW
		.then(() => {
	...
	platforms.forEach(function(platform) { // NEED TO FIND A WAY TO DO FOREACH WITH PROMISES
		tasks.push(function(next) {
	...
	async.series(tasks, function(err) { // WILL BE MERGED INTO PROMISE CHAIN WHEN THE ABOVE SEGMENT IS SETTLED
		AppcUtil.cleanNonGaSDKs(sdkPath, function(cleanupErr) {
