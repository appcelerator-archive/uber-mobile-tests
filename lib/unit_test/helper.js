	...
	Promise.resolve()
		.then(() => {
			...
			return new Promise(resolve => {
				initEnvironment(sdkSrc, resolve);
			});
		})
		.then(() => {
		...
		.then(() => {
			return new Promise(resolve => {
				build(platforms, results, resolve);
			});
		})
		.then(() => {
...
function initEnvironment(sdkSrc, done) {
	let installSDK = new Promise((resolve, reject) => {
		if (sdkSrc) {
			console.log('Installing SDK');
			AppcUtil.installSDK(sdkSrc, function(done) {
...
function build(platforms, results, done) {
	let promises = [];
