		...
		return new Promise(resolve => {
			...
			function __readPlayerLog(deltaLimit) {
				...
				playerStream.on('data', output => {
					...
					const matches = output.toString().trim().match(/.+aapt found at/g);

					// need this check because sometimes, the playerLog would not contain the 'aapt found at' entry.
					// especially, if it's a clean install of genymotion and this is the first time creating playerLog.
					if (matches) {
						const
							// try to grab the last line, since it will have the recent log entry
							lastLine = matches[matches.length - 1],
							// capture the timestamp that is prefixed in the log
