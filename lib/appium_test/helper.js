...
const
	...
	TEMP_APP = './temp',
	ALLOY_BASE = './alloy_app',
	CLASSIC_BASE = './classic_app',
...
exports.test = function (platforms, done, bail) {
	...
	_createTests(suiteData).forEach(test => {
		p = p.then(() => {
			return _createTestApp(test.app);
		})
		.then(() => {
...
function _createTestApp(testApp) {
	// verify if the testApp is a classic or alloy app; if neither, bail
	...
	files.forEach(file => {
		...
		if (file === 'Resources') {
			try {
				...
				const msg = `${testApp} is a Titanium Classic app. The app needs to have at least 'Resources/app.j'.`;
				throw new Error(msg);
			}
		}
		...
		// Alloy app
		if (file === 'app') {
			const appDir = path.join(testApp, 'app');

			let
				prefixErr = `${testApp} is an Alloy app. It needs the following:\n`,
				errMsg = '';

			try {
				const indexFile = path.join(appDir, 'controllers', 'index.js');
				fs.statSync(indexFile);
			}
			catch (err) {
				const msg = `. The app needs to have at least 'Resources/app.j'.`;
				throw new Error(msg);
			}
		}

		// TODO: check tiapp.xml
	});

	// TODO: create the TEMP_APP folder and copy assets based on the type of testApp
	// TODO: copy contents from testApp into TEMP_APP; fs-extra.copySync should be able to handle overwriting existing files/directories


	throw new Error('THROWING TO KILL THE APPIUM PROC');
}
