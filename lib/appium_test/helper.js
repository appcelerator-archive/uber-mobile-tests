	...
	.then(() => {
		return new Promise(resolve => {
			...
			playerStream.on('data', output => {
				...
				console.log('connected:', /Connected$/g.test(lastLine));
				console.log();

				// if the timestamp is within 1.5 minutes of current time and log ends with 'Connected' (launched), then move to next task
				if (deltaTime <= 90000 && /Connected$/g.test(lastLine)) {
